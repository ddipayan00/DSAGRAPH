...
public class BFSISLAND {
    ...
    public static void bfsAlgo(ArrayList<ArrayList<Integer>> adj, int s, boolean[] visted){
    ...
    public static int bfsAlgoMain(ArrayList<ArrayList<Integer>> adj){
        boolean[] visted = new boolean[adj.size()+1];
        int count = 0;
        for(int i=0;i<adj.size();i++){
            if(visted[i] == false){
                bfsAlgo(adj,i,visted);
                count++;
            }
        }
        return count;
